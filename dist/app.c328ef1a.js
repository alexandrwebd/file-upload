...
function upload(selector) {
  ...
  var files = []; // функция загрузки файлов на сервер

  var onUpload = (_options$onUpload = options.onUpload) !== null && _options$onUpload !== void 0 ? _options$onUpload : noop; //   получаю инпут

  var input = document.querySelector(selector); //создаю button, добавляю класс, добавляю контент

  var open = element('button', ['btn'], 'Открыть'); //создаю блок для картинок preivew

  var preview = element('div', ['preview']);
  var upload = element('button', ['btn', 'primary'], 'Загрузить'); // если файлов еще нету, скрываем кнопку

  upload.style.display = 'none'; //    возможность добавления нескольких картинок
  ...
  input.insertAdjacentElement("afterend", open); //    обрабатываем входящие файлы которые выбрали

  var changeHanddler = function changeHanddler(event) {
    ...
    files = Array.from(event.target.files); //очищаю блок с превью от предыдущих загрузок

    preview.innerHTML = '';
    upload.style.display = 'inline'; //прохожу цыклом, проверяю регуляркой на соответствие типу image
  ...
  var removeHandler = function removeHandler(event) {
    ...
    var name = event.target.dataset.name; //удаляю превью из памяти изменяя массив с файлами
    ...
    var block = preview.querySelector("[data-name=\"".concat(name, "\"]")) // удаляю ближащий блок с класом preview-image
    .closest('.preview-image'); //добавляю анимацию при удалении
...
_app.default.initializeApp(firebaseConfig); // получаю функцию storage
...
(0, _upload.upload)('#file', {
  ...
  onUpload: function onUpload(files, blocks) {
    ...
    files.forEach(function (file, index) {
      // получаю
      var ref = storage.ref("images/".concat(file.name)); // сохраняю

      var task = ref.put(file); // прослушиваю
